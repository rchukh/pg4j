package com.github.ltree4j.commons.hibernate.function.ltree;
...
public abstract class LTreeFunction implements SQLFunction {
    ...
    protected void prepare(@SuppressWarnings("rawtypes") List args)
	    throws QueryException {
	if (args.size() != 2) {
	...
	if (this.getFirstParam() == null || this.getFirstParam().isEmpty()
		|| this.getSecondParam() == null
		|| this.getSecondParam().isEmpty()) {
	    throw new IllegalArgumentException(
		    "The function must be passed 2 arguments with NOT_EMPTY values.");
	}
    }
